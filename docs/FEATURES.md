...
- Build includes `signing` and `org.danilopianini.publish-on-central` for Sonatype Central Publisher API flow.
- Publication contains Maven Central-required POM metadata:
- project `name`/`description`/`url`
- `licenses`
- `developers`
- `scm`
- Signing is required for Gradle publish/release tasks and supports Gradle properties `signingKey` + `signingPassword` (with backward compatibility to `signingInMemoryKey` + `signingInMemoryKeyPassword`).
...
- Runs `./gradlew publishAllPublicationsToProjectLocalRepository releaseMavenCentralPortalPublication` to bundle and release via Central Publisher API.
...
- Targets Sonatype upload endpoint: `https://central.sonatype.com/api/v1/publisher`.
